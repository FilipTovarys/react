...
export default function App() {
  ...
  function handleCreateTask(input) {
    postRequest(input)
      .then((id) => {
        ...
        const updatedTasks = [newTask, ...tasks].map((task, i) => ({
          ...task,
          order: i,
        }));
  ...
  return (
    <div className="app">
      ...
      <div id="under-input">
        {moreThanOneTask && <p>{tasksLeft} task left</p>}
      </div>
      <div>
        <div>
          {tasks
        ...
        <div id="filters">
          <button
            className={
              moreThanOneTask ? "filter-button" : "lifeless-filter-button"
            }
          ...
          >
            {doneFilterButt ? "Complete" : "All"}
          </button>
          <button
            className={
              isAtLeastOneCompleted ? "filter-button" : "lifeless-filter-button"
            }
